...
export function generatePediatricReport() {
  ...
  const avgFeedingInterval = recentFeeding.length > 1
    ? recentFeeding
        .slice(1)
        .reduce((acc, f, i) => acc + (f.timestamp - recentFeeding[i].timestamp), 0) /
  ...
  if (recentFeeding.length > 0) {
    ...
    autoTable(doc, {
      ...
      body: recentFeeding.slice(-20).reverse().map(f => [
        format(new Date(f.timestamp), 'MMM d'),
        format(new Date(f.timestamp), 'h:mm a'),
        f.type,
        f.amount ? `${f.amount} oz` : '-',
      ]),
      theme: 'grid',
      styles: { fontSize: 8 },
    });
