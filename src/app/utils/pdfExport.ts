...
const FOOTER_TEXT = 'Generated by Cradl · cradl.app';

export function generatePediatricReport(isPremium: boolean = false) {
  try {
    const doc = new jsPDF();

    // Premium: cover page with photo and subtitle
    if (isPremium) {
      doc.setFontSize(22);
      doc.text('Baby Care Summary Report', 14, 24);
      doc.setFontSize(11);
      doc.text('Report generated for GP/health visitor', 14, 32);
      const reportEnd = new Date();
      const reportStart = new Date(reportEnd.getTime() - 30 * 24 * 60 * 60 * 1000);
      doc.text(`Report period: ${format(reportStart, 'd MMM yyyy')} – ${format(reportEnd, 'd MMM yyyy')} (last 30 days)`, 14, 40);
      doc.setFontSize(10);
      doc.text(`Generated: ${format(new Date(), 'd MMMM yyyy, HH:mm')}`, 14, 48);
      ...
      if (babyProfile?.photoDataUrl && babyProfile.photoDataUrl.startsWith('data:image')) {
        try {
          doc.addImage(babyProfile.photoDataUrl, 'JPEG', 14, 56, 30, 30);
        } catch {}
      }
      doc.addPage();
    }
    ...
    // Custom trackers (last 7 days)
    const customTrackers = safeParse<{ id: string; name: string; icon: string }[]>('customTrackers', []);
    const customTrackerLogs = safeParse<{ id: string; trackerId: string; timestamp: number; value?: number | null; note?: string | null }[]>('customTrackerLogs', []);
    if (customTrackers.length > 0 && Array.isArray(customTrackers)) {
      const recentCustomLogs = customTrackerLogs.filter((e) => e.timestamp > sevenDaysAgo);
      doc.setFontSize(14);
      doc.text('Custom trackers', 14, yPos);
      yPos += 8;
      doc.setFontSize(10);
      customTrackers.forEach((t) => {
        const count = recentCustomLogs.filter((l) => l.trackerId === t.id).length;
        doc.text(`${t.name}: ${count} entries (last 7 days)`, 14, yPos);
        yPos += 6;
      });
      yPos += 4;
    }
    ...
    // Free users: watermark on page 2+
    if (!isPremium && doc.getNumberOfPages() > 1) {
      for (let p = 2; p <= doc.getNumberOfPages(); p++) {
        doc.setPage(p);
        doc.setFontSize(10);
        doc.setTextColor(200, 200, 200);
        doc.text('Upgrade to Premium for the full report.', 105, 150, { align: 'center' });
